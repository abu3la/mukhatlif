...
export function validateEpisodeNumbers(
  ...
  if (
    ...
    return {
      ok: false,
      field: 'episodeNumber',
      message: 'أدخل رقم حلقة صحيحا أكبر من صفر.',
    };
  }
...
export function validateEpisodeAudioFile(file: File): string | null {
  ...
  if (file.size === 0) return 'الملف فارغ. اختر ملفا صوتيا آخر.';
  if (file.size > MAX_AUDIO_FILE_BYTES) {
    return 'حجم الملف أكبر من 500 م.ب. اختر ملفا أصغر.';
  }
  return null;
}
...
export function getEpisodeOperationErrorMessage(
  ...
  if (
    ...
    return 'تعذر رفع الصوت. راجع الملف ثم حاول مرة أخرى.';
  }
  if (!isAdminRepositoryError(cause)) {
    return operation === 'save'
      ? 'تعذر حفظ الحلقة. حاول مرة أخرى.'
      : 'تعذر تحديث حالة الحلقة. حاول مرة أخرى.';
  }
  ...
  switch (cause.code) {
    ...
    case 'CONFLICT':
      return 'تغيرت بيانات الحلقة. حدث الصفحة ثم حاول مرة أخرى.';
    case 'NETWORK':
    case 'REMOTE_UNAVAILABLE':
    case 'RATE_LIMITED':
      return 'تعذر الاتصال بالخادم. انتظر قليلا ثم حاول مرة أخرى.';
    case 'VALIDATION':
      return operation === 'save'
        ? 'تعذر حفظ البيانات المدخلة. راجع الحقول ثم حاول مرة أخرى.'
    ...
    case 'INVALID_RESPONSE':
      return operation === 'save'
        ? 'تعذر حفظ الحلقة بسبب خطأ في الخدمة. حاول مرة أخرى.'
        : 'تعذر تحديث الحالة بسبب خطأ في الخدمة. حاول مرة أخرى.';
  }
}
